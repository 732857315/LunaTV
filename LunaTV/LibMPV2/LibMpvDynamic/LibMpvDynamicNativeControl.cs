...
public class LibMpvDynamicNativeControl : NativeControlHost
{
    ...
    protected override IPlatformHandle CreateNativeControlCore(IPlatformHandle parent)
    {
        ...
        if (!_isInitialized && Player != null)
            try
            {
                ...
                Debug.WriteLine($"Initializing mpv with native window handle: {_nativeHandle}");

                InitializeWithNativeWindow(_nativeHandle);

                _isInitialized = true;
                Debug.WriteLine("MpvPlayer initialized successfully with native window!");
            }
            catch (Exception ex)
            {
    ...
    private void InitializeWithNativeWindow(IntPtr windowHandle)
    {
        ...
        Player.SetOptionString("background-color", "#000000");
