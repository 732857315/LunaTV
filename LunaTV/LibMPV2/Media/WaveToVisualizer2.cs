...
public class WaveHeader2
{
    ...
    public WaveHeader2(Stream stream)
    {
        ...
        if (bytesRead < buffer.Length) throw new ArgumentException("Stream is too small");
        ...
        var fmtBuffer = FmtChunkSize <= 128
    ...
    ///     Size of sound data
    /// </summary>
    public uint DataChunkSize { get; }

    public int DataStartPosition { get; }
    ...
    internal static void WriteHeader(Stream toStream, int sampleRate, int numberOfChannels, int bitsPerSample,
        int sampleCount)
    {
...
public class WavePeakData2
{
    ...
    public IList<WavePeak2> Peaks { get; }
    ...
    {
        if (Peaks is WavePeak2[] array) return array;

        if (Peaks is List<WavePeak2> list) return CollectionsMarshal.AsSpan(list);
    ...
    private void CalculateHighestPeak()
    {
        ...
        {
            var abs = peak.Abs;
            if (abs > HighestPeak) HighestPeak = abs;
        }
    }
...
public class SpectrogramData2 : IDisposable
{
    ...
    public bool IsLoaded => _loadFromFilePath == null && _rawSamples == null;
    ...
    public void Load()
    {
        ...
        {
            var filePath = _loadFromFilePath;
            ...
            {
                Debug.Write(exception, $"Unable to load spectrogram from {filePath}");
            }
        }
    }
    ...
    private void GenerateImagesFromRawData()
    {
        ...
        Parallel.For(0, chunkCount, new ParallelOptions { MaxDegreeOfParallelism = Environment.ProcessorCount },
            ...
            (iChunk, loopState, drawer) =>
            {
                ...
                for (var i = 0; i < chunkSampleCount; i++) chunkSamples[i] = _rawSamples[offset + i];
    ...
    public static void SaveToBinaryFile(string filePath, int fftSize, int imageWidth, double sampleDuration,
        float[] samples)
    {
...
public class WavePeakGenerator2 : IDisposable
{
    ...
    public WavePeakGenerator2(string fileName)
        : this(new FileStream(fileName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite, 65536,
    ...
    {
        var dir = GlobalDefine.WaveformsFolder;
    ...
    {
        var peaksPerSecond = Math.Min(GlobalDefine.WaveformMinimumSampleRate, Header.SampleRate);

        // ensure that peaks per second is a factor of the sample rate
        while (Header.SampleRate % peaksPerSecond != 0) peaksPerSecond++;

        const int timeCodeBaseUnit = 1000;
        var delaySampleCount = Header.SampleRate * (delayInMilliseconds / timeCodeBaseUnit);
        ...
        var fileSampleCount = Header.LengthInSamples;
        long fileSampleOffset = -delaySampleCount;
        var chunkSampleCount = Header.SampleRate / peaksPerSecond;
        var peaks = new List<WavePeak2>((int)((fileSampleCount + delaySampleCount + chunkSampleCount - 1) /
                                              chunkSampleCount));
        var data = new byte[chunkSampleCount * Header.BlockAlign];
        ...
        if (fileSampleOffset > 0) _stream.Seek(fileSampleOffset * Header.BlockAlign, SeekOrigin.Current);
        ...
        if (Header.BytesPerSample == 2 && Header.NumberOfChannels == 2) // optimized path for 16-bit stereo PCM
            while (fileSampleOffset < fileSampleCount)
            {
                ...
                {
                    var fileReadByteCount = fileReadSampleCount * Header.BlockAlign;
            ...
            while (fileSampleOffset < fileSampleCount)
            {
                ...
                if (fileReadSampleCount > 0)
                {
                    ...
                    while (dataByteOffset < fileReadByteCount)
                    {
                        ...
                        for (var iChannel = 0; iChannel < Header.NumberOfChannels; iChannel++)
                        {
    ...
    {
        var peaksPerSecond = GlobalDefine.WaveformMinimumSampleRate;
        var peaks = new List<WavePeak2>
        {
            new(1000, -1000)
        };
        var totalPeaks = peaksPerSecond * totalSeconds;
        for (var i = 0; i < totalPeaks; i++) peaks.Add(new WavePeak2(1, -1));
    ...
    {
        var result = Unsafe.ReadUnaligned<int>(ref data[index]);
        index += 4;
        return result;
    }
    ...
    public SpectrogramData2 GenerateSpectrogram(int delayInMilliseconds, string spectrogramFilePath,
        CancellationToken token)
    {
        ...
        var readSampleDataValue = GetSampleDataReader();
        var sampleAndChannelScale = GetSampleAndChannelScale();
        var fileSampleCount = Header.LengthInSamples;
        long fileSampleOffset = -delaySampleCount;
        var chunkSampleCount = fftSize * imageWidth;
        var chunkCount = (int)Math.Ceiling((double)(fileSampleCount + delaySampleCount) / chunkSampleCount);
        var data = new byte[chunkSampleCount * Header.BlockAlign];
        var allSamples = new float[chunkCount * chunkSampleCount];
        var allSamplesOffset = 0;
        ...
        for (var iChunk = 0; iChunk < chunkCount; iChunk++)
        {
            ...
            // calculate how many samples to read from the file
            var fileSamplesRemaining = fileSampleCount - Math.Max(fileSampleOffset, 0);
            var fileReadSampleCount = (int)Math.Min(fileSamplesRemaining, chunkSampleCount - startPaddingSampleCount);

            // calculate padding at the end (when the data isn't an even multiple of our chunk size)
            var endPaddingSampleCount = chunkSampleCount - startPaddingSampleCount - fileReadSampleCount;
        ...
        var sampleDuration = (double)fftSize / Header.SampleRate;
    ...
    public class SpectrogramDrawer
    {
        ...
        public SpectrogramDrawer(int nfft, SKColor[] palette)
        {
            ...
            var scaleCorrection = 1.0 / (RaisedCosineWindowScale * _fft.ForwardScaleFactor);
            for (var i = 0; i < _window.Length; i++) _window[i] *= scaleCorrection;
        }

        public static string GetSpectrogramFileName(string videoFileName, int trackNumber = -1)
        {
            var dir = GlobalDefine.SpectrogramsFolder;
            if (!Directory.Exists(dir)) Directory.CreateDirectory(dir);
            ...
            var files = Directory.GetFiles(dir, $"{hash}_*.spectrogram")
                .OrderBy(p => p)
                .ToList();
            if (files.Count > 0 && trackNumber < 0) return files[0];
        ...
        {
            var width = samples.Length / _nfft;
            var height = _nfft / 2;
            ...
            for (var x = 0; x < width; x++)
            {
                ...
                {
                    var colorIndex = _mapper.Map((_magnitude1[y] + _magnitude2[y]) / 2.0);
        ...
        private void ProcessSegment(double[] samples, int offset, double[] magnitude)
        {
            ...
            for (var i = 0; i < _nfft; i++) _segment[i] = samples[offset + i] * _window[i];
        ...
        {
            var twoPiOverN = Math.PI * 2.0 / n;
            var dst = new double[n];
            for (var i = 0; i < n; i++) dst[i] = 0.5 * (1.0 - Math.Cos(twoPiOverN * i));
        ...
        public static SKColor[] GeneratePaletteForCurrentStyle()
        {
            ...
            if (GlobalDefine.SpectrogramStyle == SeSpectrogramStyle.ClassicViridis.ToString())
                for (var colorIndex = 0; colorIndex < MagnitudeIndexRange; colorIndex++)
                    palette[colorIndex] = PaletteValueViridis(colorIndex, MagnitudeIndexRange);
            else if (GlobalDefine.SpectrogramStyle == SeSpectrogramStyle.ClassicPlasma.ToString())
                for (var colorIndex = 0; colorIndex < MagnitudeIndexRange; colorIndex++)
                    palette[colorIndex] = PaletteValuePlasma(colorIndex, MagnitudeIndexRange);
            else if (GlobalDefine.SpectrogramStyle == SeSpectrogramStyle.ClassicInferno.ToString())
                for (var colorIndex = 0; colorIndex < MagnitudeIndexRange; colorIndex++)
                    palette[colorIndex] = PaletteValueInferno(colorIndex, MagnitudeIndexRange);
            else if (GlobalDefine.SpectrogramStyle == SeSpectrogramStyle.ClassicTurbo.ToString())
                for (var colorIndex = 0; colorIndex < MagnitudeIndexRange; colorIndex++)
                    palette[colorIndex] = PaletteValueTurbo(colorIndex, MagnitudeIndexRange);
            else if (GlobalDefine.SpectrogramStyle == SeSpectrogramStyle.Neon.ToString())
                for (var colorIndex = 0; colorIndex < MagnitudeIndexRange; colorIndex++)
        ...
        private static SKColor PaletteValuePlasma(int x, int range)
        {
            ...
            var r = 0.050383 + t * (2.176514 +
                                    t * (-2.689460 +
                                         t * (6.130348 + t * (-11.107290 + t * (10.024779 + t * -3.657430)))));
            var g = 0.029803 + t * (0.280267 + t * (2.645293 + t * (-5.336825 + t * (4.481445 + t * -1.355430))));
            var b = 0.527975 + t * (0.600417 + t * (1.412440 + t * (-11.930240 + t * (20.434160 + t * -12.791690))));

            // Fade to black at low magnitudes so silence renders as black rather than blue
            var fade = Math.Min(1.0, t / 0.15);
            r *= fade;
            g *= fade;
            b *= fade;
        ...
        private static SKColor PaletteValueInferno(int x, int range)
        {
            ...
            var r = 0.001462 + t * (1.217761 +
                                    t * (1.795470 +
                                         t * (-7.361869 + t * (13.446884 + t * (-9.555991 + t * 2.455710)))));
            var g = 0.000466 + t * (0.125098 + t * (3.875940 + t * (-10.418160 + t * (11.001100 + t * -4.909755))));
            var b = 0.013866 + t * (2.565590 + t * (-6.945260 + t * (9.287860 + t * (-5.684940 + t * 1.316750))));
        ...
        private class MagnitudeToIndexMapper
        {
            ...
            {
                var mappingScale = indexMax / decibelRange;
            ...
            public static int Map(double magnitude, double decibelRange, int indexMax)
            {
                ...
                var decibelLevel = 20.0 * Math.Log10(magnitude);
                return decibelLevel >= -decibelRange
                    ? (int)(indexMax * (decibelLevel + decibelRange) / decibelRange)
                    : 0;
            }
        }
    }
