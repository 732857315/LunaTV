...
public static class FindVideoFileName
{
    public static bool TryFindVideoFileName(string inputFileName, out string videoFileName,
        bool openSearchParentFolder = true)
    {
        videoFileName = string.Empty;

        if (string.IsNullOrWhiteSpace(inputFileName)) return false;
    ...
    private static bool TryFindVideoFileNameInner(string inputFileName, out string videoFileName,
        HashSet<string> visitedPaths)
    {
        ...
        if (visitedPaths.Contains(inputFileName)) return false;
        ...
        if (lastDotIndex > 0)
        {
            ...
            if (TryFindVideoFileNameInner(withoutExtension, out videoFileName, visitedPaths)) return true;
        }
        ...
        if (lastUnderscoreIndex > 0)
        {
            ...
            if (TryFindVideoFileNameInner(withoutSuffix, out videoFileName, visitedPaths)) return true;
        }
