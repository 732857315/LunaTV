...
internal static class NativeMethods
{
    ...
    private static readonly IntPtr _libdlHandle;
    private static readonly IntPtr _libcHandle;
    ...
    static NativeMethods()
    {
        if (!RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            ...
            {
                string[] libSystemNames = new[] { "libSystem.dylib", "libSystem.B.dylib" };
                foreach (string name in libSystemNames)
                {
            ...
            else
            {
                ...
                string[] libcNames = new[] { "libc.so.6", "libc.so", "libc" };
                foreach (string name in libcNames)
                {
                    if (NativeLibrary.TryLoad(name, out _libcHandle))
                    {
                        break;
                ...
                string[] libdlNames = new[] { "libdl.so.2", "libdl.so", "libdl" };
                foreach (string name in libdlNames)
                {
            ...
            {
                if (NativeLibrary.TryGetExport(_libcHandle, "setlocale", out IntPtr setlocalePtr))
                {
            ...
            if (_libdlHandle != IntPtr.Zero)
            {
                if (NativeLibrary.TryGetExport(_libdlHandle, "dlopen", out IntPtr dlopenPtr))
                {
                    _dlopen = Marshal.GetDelegateForFunctionPointer<DlopenDelegate>(dlopenPtr);
                }
                if (NativeLibrary.TryGetExport(_libdlHandle, "dlclose", out IntPtr dlclosePtr))
                {
                    _dlclose = Marshal.GetDelegateForFunctionPointer<DlcloseDelegate>(dlclosePtr);
                }
                if (NativeLibrary.TryGetExport(_libdlHandle, "dlsym", out IntPtr dlsymPtr))
                {
                    _dlsym = Marshal.GetDelegateForFunctionPointer<DlsymDelegate>(dlsymPtr);
                }
                if (NativeLibrary.TryGetExport(_libdlHandle, "dlerror", out IntPtr dlerrorPtr))
                {
    ...
    internal static string? dlerror()
    {
        ...
        IntPtr ptr = _dlerror?.Invoke() ?? IntPtr.Zero;
    ...
    internal static IntPtr CrossLoadLibrary(string fileName)
    {
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            ...
            try
            {
                ...
                if (handle == IntPtr.Zero)
                {
                    ...
                    Debug.WriteLine($"Failed to load {fileName}, error code: {error}");
                }
            }
            finally
            {
        ...
        if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
        {
            ...
            {
                Debug.WriteLine($"Successfully loaded {fileName} using NativeLibrary.TryLoad");
                return handle;
            }

            Debug.WriteLine($"NativeLibrary.TryLoad failed for {fileName}, trying dlopen strategies");

            // Strategy 1: Try RTLD_LAZY | RTLD_GLOBAL (more permissive, allows undefined symbols)
            IntPtr result = dlopen(fileName, RTLD_LAZY | RTLD_GLOBAL);
            if (result != IntPtr.Zero)
            {
                Debug.WriteLine($"Successfully loaded {fileName} with dlopen (RTLD_LAZY | RTLD_GLOBAL)");
                return result;
            }
            string? error1 = dlerror();
            Debug.WriteLine($"dlopen with RTLD_LAZY | RTLD_GLOBAL failed: {error1}");

            // Strategy 2: Try RTLD_NOW | RTLD_GLOBAL (resolve all symbols immediately)
            result = dlopen(fileName, RTLD_NOW | RTLD_GLOBAL);
            if (result != IntPtr.Zero)
            {
                Debug.WriteLine($"Successfully loaded {fileName} with dlopen (RTLD_NOW | RTLD_GLOBAL)");
                return result;
            }
            string? error2 = dlerror();
            Debug.WriteLine($"dlopen with RTLD_NOW | RTLD_GLOBAL failed: {error2}");

            // Strategy 3: Try with RTLD_LAZY | RTLD_LOCAL
            result = dlopen(fileName, RTLD_LAZY | RTLD_LOCAL);
            if (result != IntPtr.Zero)
            {
                Debug.WriteLine($"Successfully loaded {fileName} with dlopen (RTLD_LAZY | RTLD_LOCAL)");
                return result;
            }
            string? error3 = dlerror();
            Debug.WriteLine($"dlopen with RTLD_LAZY | RTLD_LOCAL failed: {error3}");

            Debug.WriteLine($"All dlopen strategies failed for {fileName}");
            return IntPtr.Zero;
        }
    ...
    {
        IntPtr address = CrossGetProcAddress(handle, name);
