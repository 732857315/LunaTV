...
public class WavePeakData2
{
    ...
    {
        HighestPeak = 0;
        foreach (WavePeak2 peak in Peaks)
        {
            int abs = peak.Abs;
            if (abs > HighestPeak)
            {
...
public class WavePeakGenerator2 : IDisposable
{
    ...
    public WavePeakGenerator2(string fileName)
        : this(new FileStream(fileName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite, 65536, FileOptions.SequentialScan))
    {
    }
    ...
    {
        string dir = GlobalDefine.WaveformsFolder;
        ...
        string hash = MovieHasher.GenerateHash(videoFileName);

        if (trackNumber < 0)
        {
            string bare = Path.Combine(dir, $"{hash}.wav");
            if (File.Exists(bare))
            {
                return bare;
            }
        ...
        string wavePeakName = trackNumber >= 0 ? $"{hash}-{trackNumber}.wav" : $"{hash}.wav";
    ...
    {
        int peaksPerSecond = Math.Min(GlobalDefine.WaveformMinimumSampleRate, Header.SampleRate);
        ...
        int delaySampleCount = Header.SampleRate * (delayInMilliseconds / timeCodeBaseUnit);
        ...
        ReadSampleDataValue readSampleDataValue = GetSampleDataReader();
            ...
            while (fileSampleOffset < fileSampleCount)
            {
                ...
                if (fileReadSampleCount > 0)
                {
                    ...
                    while (dataByteOffset < fileReadByteCount)
                    {
                        ...
                        {
                            int v = readSampleDataValue(data, ref dataByteOffset);
                            if (v < 0)
                            {
                                valueNegative += v;
                            }
        ...
        {
            using (FileStream stream = File.Create(peakFileName))
            {
    ...
    {
        int peaksPerSecond = GlobalDefine.WaveformMinimumSampleRate;
        var peaks = new List<WavePeak2>
        {
            new(1000, -1000)
        };
        int totalPeaks = peaksPerSecond * totalSeconds;
        ...
        // save results to file
        string? dir = Path.GetDirectoryName(peakFileName);
        if (!string.IsNullOrEmpty(dir) && !Directory.Exists(dir))
        {
            Directory.CreateDirectory(dir);
        }
    ...
    public SpectrogramData2 GenerateSpectrogram(int delayInMilliseconds, string spectrogramFilePath, CancellationToken token)
    {
    ...
    public class SpectrogramDrawer
    {
        ...
        {
            string dir = GlobalDefine.SpectrogramsFolder;
            if (!Directory.Exists(dir))
            {
                Directory.CreateDirectory(dir);
            }
            ...
            string spectrogramFileName = trackNumber >= 0 ? $"{hash}-{trackNumber}.spectrogram" : $"{hash}.spectrogram";
